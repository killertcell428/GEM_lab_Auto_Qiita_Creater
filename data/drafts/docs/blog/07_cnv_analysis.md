...
Control-FREECは、**リード深度（coverage）を基にCNVを検出**するツールです。
...
1. **CNV領域の定義**：BEDファイルでCNV領域を定義
2. **期待コピー数の設計**：性差・亜種差を反映
3. **検出値のシミュレーション**：期待値にノイズを加えて検出値を生成
